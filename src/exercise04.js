function countBuildingsWithViewO4(heights) {  // So this is a function that takes in an array of building heights and counts how many buildings have a view to the sea. The view is unobstructed if there are no buildings taller than the current one to its left.
  ...
  for (let i = heights.length - 1; i >= 0; i--) { // So we're going to loop through the array in reverse order, from right to left.
      if (heights[i] > max_height) { // So if the height of the current building is greater than the maximum height we've seen so far, then we're going to increment the counter and update the maximum height.
...
console.log(countBuildingsWithViewO4([3, 7, 8, 3, 6, 1])); // So we're going to call the function with an array of building heights.
console.log(countBuildingsWithViewO4([1, 4, 5, 8])); // 1

/*This JavaScript function countBuildingsWithViewO4 takes an array of building heights and counts how many buildings have a view
 to the sea. The view is unobstructed if there are no buildings taller than the current one to its left.
